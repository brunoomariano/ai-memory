...
| `AI_MEMORY_EMBEDDING_PROVIDER=openai` + `AI_MEMORY_EMBEDDING_BASE_URL=https://openrouter.ai/api/v1` | `openai/text-embedding-3-small` via [OpenRouter](https://openrouter.ai) | Same API key as `LLM_API_KEY` / `OPENAI_API_KEY`; no código extra além do cliente OpenAI-compat. |
